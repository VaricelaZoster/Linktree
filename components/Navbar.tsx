    ...
    return (
        <div className='fixed top-10 z-10 w-[94%] bg-white h-12 md:h-21 mx-17 px-15 py-2 rounded-full flex items-center justify-between'>
            ...
            <div className='flex justify-between gap-3 text-sm md:text-lg font-[500]'>
                <div onClick={() => router.push('/login')} className='bg-[#eff0ec] hover:bg-[#e9e9e9] flex items-center md:p-4 md:px-6 rounded-lg duration-150 cursor-pointer'>Log in</div>
                <div onClick={() => router.push('/signup')} className='bg-[#1e2330] hover:bg-[#262d3e] flex items-center p-4 px-6 rounded-full text-white cursor-pointer duration-150'>Sign up free</div>
                <div className='flex items-center flex [@media(min-width:1133px)]:hidden'><Bars3Icon className='h-9'/></div>
            </div>
        </div>
    )
}
